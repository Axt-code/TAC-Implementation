...
def connect_to_ethereum_node():
    ...
    w3 = Web3(Web3.HTTPProvider("http://127.0.0.1:7546", request_kwargs={'timeout': 300}))
...
def blinding_attribute(private_attribute):
    ...
    m_0 = m_0 or random.randint(2, o)
    # print(f"first m_0: {m_0}")
    # print(f"H: {H}")
...
def make_pi_a( ):
    ...
    k = add(multiply(H[0], r[0]),  ec_sum([multiply(H[i+1], r[i+1]) for i in range(len(private_attribute))]))
...
def make_B(public_attribute, private_attribute,):
    global m_0, H, B
    t = add(multiply(H[0], m_0),  ec_sum([multiply(H[i+1], (ai)%o) for i, ai in enumerate(private_attribute)]))  
...
def verify_signature():
    ...
    global U, B, A, public_attribute, private_attribute, e1, H, X, o, R, U, m_0
...
def broadcast_show_cred(contract, proof, public_attribute, total_attributes , user_address):
    global H, X

    (A_bar,B_bar,c,s,t,u_i,_timestamp) = proof                                  

    A_bar = (A_bar[0].n, A_bar[1].n)
    B_bar = (B_bar[0].n, B_bar[1].n)
    pi = (c, s, t, u_i, _timestamp)
    ...
    try:
        tx_hash = contract.functions.broadcastSPOK(sid, sigid, _H, _X, pi, A_bar, B_bar, public_attribute, total_attributes).transact({'from': user_address})
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=300)
        if receipt.status == 1:
...
def main():
    ...
    # Step 8: Unblind and verify the signature
    verify_signature()


    take_time(f"User start_time for make_spok", time.time())
    private_attribute.insert(0, m_0)
    total_attributes = len(private_attribute) + len(public_attribute)
    sign=(A,e1)

    proof = make_spok(private_attribute, public_attribute, H, sign)
  
    broadcast_show_cred(SPOK_contract, proof, public_attribute, total_attributes , user_address)
