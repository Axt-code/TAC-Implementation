...
contract RequestCredential {
    ...
    function verify_pi_a(
        ...
        for (uint256 i = 1; i < kk.length; i++) {
            k1 = G.g1add(k1, G.g1mul(_H[i], kk[i]));
        }

        G.G1Point memory k = G.g1add(k0, k1);

        // uint256[2] memory _k1 = [k1.X, k1.Y];
        // uint256[2] memory _k0 = [k0.X, k0.Y];
        // uint256[2] memory _k = [k.X, k.Y];

        // uint256 minus_c = G.GEN_ORDER - c;
        // emit debug(_k1, _k0, _k, c, minus_c);
