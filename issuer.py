...
# Utility Function: Read Specific Line from File
def get_line_content(file_path, line_number):
    try:
        with open(file_path, 'r') as file:
            lines = file.readlines()
            return lines[line_number].strip() if len(lines) > line_number else None
    except IndexError:
        return None
...
async def mpc_compute_partial_cred():
    global sk, issuer_id, e, s, r_value, u_i, o, lemda_i
    issuers = len(mpc.parties)

    secnum = mpc.SecFld(o)
    secure_e_value = secnum(random.randint(2, o))
    secure_s_value = secnum(random.randint(2, o))
    ...
    # print_with_timestamp(f"Partial Credential: e={e}, s={s}, u_i={u_i}, r_value={r_value}")


# Blockchain Functions
def issue_partial_credential():
    global r_i, u_i, s_id, sig_id, e, s, pk, issuer_id, B_lemda, pk
    _r_i = (r_i[0].n, r_i[1].n)
    _B_lemda = (B_lemda[0].n, B_lemda[1].n)
    transaction = issuer_contract.functions.issuePartialCredential(s_id, sig_id, issuer_id, e, s, u_i, _r_i, _B_lemda, pk).transact({
        'from': issuer_address,
    })
...
def listen_broadcast_sig_req():
    ...
    try:
        ...
        if entries:
            ...
            k = entries[0]['args']['k']
            c = entries[0]['args']['c']
            # print_with_timestamp("attributes: ", attributes)
        else:
...
def compute_r_i_and_B_lemda():
    ...
    B = compute_B(params, s, H_values_G1, no_of_private_attribute, B_dash_int, attributes )
